...
def create_product(
    product_in: ProductCreate, service: ProductService = Depends(get_service)
):
...
def get_products(
    skip: int = 0, limit: int = 100, service: ProductService = Depends(get_service)
):
    return service.get_all(skip=skip, limit=limit)


@router.get("/{product_id}", response_model=ProductResponse)
def read_product(product_id: int, service: ProductService = Depends(get_service)):
