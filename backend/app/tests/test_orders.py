...
def test_create_order_complete_flow(client):
    prod1 = client.post("/products/", json={"name": "Bolinho de Charque", "price": 10.0}).json()
    prod2 = client.post("/products/", json={"name": "Bolinho de Queijo", "price": 5.0}).json()
...
def test_get_all_orders(client):
    prod = client.post("/products/", json={"name": "Empada de Palmito", "price": 9.0}).json()
